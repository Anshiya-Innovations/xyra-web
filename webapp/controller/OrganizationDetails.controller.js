  ...
  function (
    ...
    function slaSummaryText(s) {
      return (
        ...
        " business days  ·  " +
        "Escalation delay: " +
        s.escalationDelayDays +
        " business days past Reviewer 2 SLA"
      );
    }

    function orgStatusStateFor(sStatus) {
      if (sStatus === "Pending Setup") {
        return "Warning";
      }
      if (sStatus === "Under Audit Review") {
        return "Information";
      }
      if (sStatus === "Inactive") {
        return "Error";
    ...
    return Controller.extend("xyraweb.controller.OrganizationDetails", {
      onInit: function () {
        var oRouter =
          ...
          (this.getOwnerComponent() && this.getOwnerComponent().getRouter());
        if (oRouter) {
      ...
      _loadSlaSettings: function () {
        ...
        return fetch(
          Config.AUTH_BASE_URL + "/api/system-config/getSlaSettings",
          {
            method: "POST",
          ...
          .then(function (oData) {
            ...
            if (oModel) {
              oModel.setProperty("/slaSummary", slaSummaryText(oData.settings));
            }
      ...
      onOpenSlaSettingsDialog: function () {
        ...
        this._loadSlaSettings().then(function (s) {
          ...
          if (!s) {
            MessageBox.error(
              "Could not load SLA settings. Is xyra-core running?",
            );
            return;
          }
      ...
      onSaveSlaSettings: function () {
        ...
        var oPayload = {
          ...
          escalationDelayDays: parseInt(
            this.byId("slaEscalationDelayDays").getValue(),
            10,
          ),
        ...
        fetch(Config.AUTH_BASE_URL + "/api/system-config/updateSlaSettings", {
          ...
          .then(function (oData) {
            ...
            if (!oData.success) {
              MessageBox.error(
                oData.message || "Could not update SLA settings.",
              );
              return;
            }
            var oModel = that.getView().getModel("orgDetailsModel");
            if (oModel) {
              oModel.setProperty("/slaSummary", slaSummaryText(oPayload));
            }
      ...
      _loadOrgData: function (sOrgId) {
        ...
        var aParameters = [
          {
            paramType: "SET/GET Parameter",
            paramIdName: "BUK - Company Code",
            value: "1000",
            status: "Enforced",
            statusState: "Success",
          },
          ...
          {
            paramType: "SET/GET Parameter",
            paramIdName: "VKO - Sales Organization",
            value: "1000",
            status: "Active",
            statusState: "Success",
          },
          {
            paramType: "SET/GET Parameter",
            paramIdName: "VTEG - Distribution Channel",
            value: "10",
            status: "Active",
            statusState: "Success",
          },
          ...
          {
            paramType: "SET/GET Parameter",
            paramIdName: "EKO - Purchasing Organization",
            value: "PO_1000",
            status: "Active",
            statusState: "Success",
          },
          {
            paramType: "User Default Value",
            paramIdName: "Decimal Notation",
            value: "1,234,567.89",
            status: "Enforced",
            statusState: "Success",
          },
          ...
          {
            paramType: "User Default Value",
            paramIdName: "Output Device (PRINTER)",
            value: "PRN01_MUMBAI",
        ...
        var oDetailsModel = new JSONModel({
          ...
          jiraStatusOptions: [
            { key: "", text: "-- Use Jira's default category --" },
          ],
          orgSystems: [],
        });
      ...
      _fetchOrganization: function (sOrgId, oDetailsModel) {
        ...
        var pOrgId = sOrgId
          ...
          : fetch(
              ...
              .then(function (oData) {
                return oData.success && oData.organizations[0]
                  ? oData.organizations[0].id
                  : null;
              });
        ...
        pOrgId
          .then(function (sId) {
            if (!sId) {
              MessageBox.error(
                "No organization to show yet - add one from the Organization list.",
              );
              return;
            }
            return fetch(
              Config.AUTH_BASE_URL + "/api/organization/getOrganization",
              {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
              ...
              .then(function (oData) {
                if (!oData.success || !oData.organization) {
                  MessageBox.error(
                    oData.message || "Could not load organization.",
                  );
                  return;
                }
                ...
                oDetailsModel.setProperty("/currentOrg", {
                  ...
                  sapSystems:
                    o.systemCount +
                    (o.systemCount === 1 ? " System" : " Systems"),
                  status: o.status,
                  statusState: orgStatusStateFor(o.status),
                });
                return Promise.all([
          ...
          .catch(function () {
            MessageToast.show("Could not reach xyra-core. Is it running?", {
              duration: 4000,
            });
          })
          .then(function () {
            ...
            var aRows = (oData.systems || [])
              ...
              .map(function (s) {
                return {
                  ...
                  connectionState:
                    s.lastConnectionStatus === "ONLINE"
                      ? "Success"
                      : s.lastConnectionStatus === "OFFLINE"
                        ? "Error"
                        : "None",
                };
              });
      ...
      _loadJiraSettings: function (sOrgId, oDetailsModel) {
        ...
        return fetch(
          Config.AUTH_BASE_URL + "/api/organization/getJiraSettings",
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
          ...
          .then(
            function (oData) {
              ...
              this._mergeJiraStatusOptions(
                oDetailsModel,
                [
                  oData.settings.statusCreated,
                  oData.settings.statusInProgress,
                  oData.settings.statusResolved,
                ].filter(Boolean),
              );
            }.bind(this),
          );
      ...
      onFetchJiraStatuses: function () {
        var that = this;
        var oSession = Session.get();
        if (!oSession || !this._orgDbId) {
          MessageBox.error("No active session or organization.");
          return;
        }
        ...
        var oPayload = {
          ...
          projectKey: (
            this.byId("jiraProjectKeyInput").getValue() || ""
          ).trim(),
        };
        ...
        fetch(Config.AUTH_BASE_URL + "/api/organization/getJiraStatusOptions", {
          ...
          .then(function (oData) {
            ...
            if (!oData.success) {
              MessageBox.error(
                oData.message || "Could not fetch statuses from Jira.",
              );
              return;
            }
            that._mergeJiraStatusOptions(
              that.getView().getModel("orgDetailsModel"),
              oData.statuses || [],
            );
            MessageToast.show(
              (oData.statuses || []).length + " status(es) loaded from Jira.",
            );
          })
          .catch(function () {
            GlobalLoading.hide();
      ...
      onSaveJiraSettings: function () {
        ...
        var oSession = Session.get();
        if (!oSession || !this._orgDbId) {
          MessageBox.error("No active session or organization to update.");
          return;
        }
        ...
        if (bEnabled && (!sSiteUrl || !sEmail || !sProjectKey)) {
          MessageBox.error(
            "Site URL, Email, and Project Key are required to enable Jira ticket creation.",
          );
          return;
        }
        ...
        var oPayload = {
          ...
          issueType: (
            this.byId("jiraIssueTypeInput").getValue() || "Task"
          ).trim(),
          statusCreated: this.byId("jiraStatusCreatedSelect").getSelectedKey(),
          statusInProgress: this.byId(
            "jiraStatusInProgressSelect",
          ).getSelectedKey(),
          statusResolved: this.byId(
            "jiraStatusResolvedSelect",
          ).getSelectedKey(),
          performedBy: oSession.email,
          performedByRole: oSession.role,
        };
        ...
        fetch(Config.AUTH_BASE_URL + "/api/organization/updateJiraSettings", {
          ...
          .then(function (oData) {
            ...
            if (!oData.success) {
              MessageBox.error(
                oData.message || "Could not update Jira settings.",
              );
              return;
            }
            that.byId("jiraApiTokenInput").setValue("");
            that._loadJiraSettings(
              that._orgDbId,
              that.getView().getModel("orgDetailsModel"),
            );
      ...
      onOpenAddParamDialog: function () {
        ...
        this.byId("newParamIdSetGetSelect")
          .setVisible(true)
          .setSelectedKey("BUK - Company Code");
        this.byId("newParamIdUserDefSelect")
          .setVisible(false)
          .setSelectedKey("Decimal Notation");
      ...
      onSubmitAddParam: function () {
        ...
        if (!sValue) {
          MessageBox.error(
            "Please enter a configured value for the parameter.",
          );
          return;
        }

        var sState = "Success";
        if (sStatus === "Pending Verification") {
          sState = "Warning";
        }
        ...
        MessageToast.show(
          "Parameter '" + sParamIdName + "' added successfully!",
        );
      },
      ...
      onRemoveParameter: function (oEvent) {
        ...
        if (oContext) {
          ...
          oModel.setProperty(
            "/allParameters",
            JSON.parse(JSON.stringify(aParams)),
          );
      ...
      onEditSummaryPress: function () {
        ...
        this.byId("editSummaryCompanyNameInput")
          .setValue(oOrg.companyName || "")
          .setValueState("None");
        this.byId("editSummaryOrgIdInput")
          .setValue(oOrg.orgId || "")
          .setValueState("None");
        this.byId("editSummaryIndustrySelect").setSelectedKey(
          oOrg.industry || "Conglomerate & Technology",
        );
        this.byId("editSummaryRegionSelect").setSelectedKey(
          oOrg.region || "Asia Pacific",
        );
        this.byId("editSummaryCountryInput")
          .setValue(oOrg.country || "")
          .setValueState("None");
        this.byId("editSummaryContactInput")
          .setValue(oOrg.primaryContact || "")
          .setValueState("None");
        this.byId("editSummaryEmailInput")
          .setValue(oOrg.email || "")
          .setValueState("None");
        this.byId("editSummaryPhoneInput")
          .setValue(oOrg.phone || "")
          .setValueState("None");
        this.byId("editSummaryStatusSelect").setSelectedKey(
          oOrg.status || "Active",
        );
      ...
      onSubmitEditSummary: function () {
        var sCompanyName = (
          this.byId("editSummaryCompanyNameInput").getValue() || ""
        ).trim();
        var sOrgId = (
          this.byId("editSummaryOrgIdInput").getValue() || ""
        ).trim();
        ...
        var sCountry = (
          this.byId("editSummaryCountryInput").getValue() || ""
        ).trim();
        var sContact = (
          this.byId("editSummaryContactInput").getValue() || ""
        ).trim();
        var sEmail = (
          this.byId("editSummaryEmailInput").getValue() || ""
        ).trim();
        var sPhone = (
          this.byId("editSummaryPhoneInput").getValue() || ""
        ).trim();
        ...
        if (!sCompanyName) {
          this.byId("editSummaryCompanyNameInput")
            .setValueState("Error")
            .setValueStateText("Company Name is required.");
          bValid = false;
        } else {
        ...
        if (!sOrgId) {
          this.byId("editSummaryOrgIdInput")
            .setValueState("Error")
            .setValueStateText("Organization ID is required.");
          bValid = false;
        } else {
        ...
        if (!sCountry) {
          this.byId("editSummaryCountryInput")
            .setValueState("Error")
            .setValueStateText("Country is required.");
          bValid = false;
        } else {
        ...
        if (!sContact) {
          this.byId("editSummaryContactInput")
            .setValueState("Error")
            .setValueStateText("Primary Contact is required.");
          bValid = false;
        } else {
        ...
        if (!sEmail || sEmail.indexOf("@") === -1) {
          this.byId("editSummaryEmailInput")
            .setValueState("Error")
            .setValueStateText("Valid Email address is required.");
          bValid = false;
        } else {
        ...
        fetch(Config.AUTH_BASE_URL + "/api/organization/updateOrganization", {
          ...
          .then(function (oData) {
            ...
            if (!oData.success) {
              MessageBox.error(
                oData.message || "Could not update organization.",
              );
              return;
            }
            ...
            MessageToast.show(
              "Company Executive Summary updated successfully!",
            );
          })
          .catch(function () {
            GlobalLoading.hide();
